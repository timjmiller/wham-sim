...
for(i in 1:length(ids)){
  m1 <- readRDS(file.path("results",paste0(ids[i],"_NAA"),"m1.rds"))
...
```{r snemayt-naa, echo=FALSE, message=FALSE, warnings=FALSE, out.width='7in',fig.height=10, fig.width=7}
id = "SNEMAYT_NAA"
...
#               select(om, em, SSB.rel, F.rel, relB.rel, relF.rel, catch.rel, R.rel) %>%
#               pivot_longer(-om, names_to = "variable", values_to = "val")
	df.plot <- filter(results, type==levels(results$type)[ty])
