...
where $N_{a,y}$ are the numbers at age *a* in year *y*, *Z* is the total mortality rate ($F + M$), $f$ is the stock-recruit function, and *A* represents the plus-group. The $\varepsilon_{a,y}$ deviations are akin to survival, which is clearly related to $M_{a,y}$, but they can also be caused by deviations in $F_{a,y}$, $W_{a,y}$, or movement into or out of the stock [@schnute1995Influence; @gudmundsson2012Selection; @stock2021Implementing]. More precisely, they arise from inconsistencies between the data and the model predicted catch (i.e. Eqn. \ref{eq:predcatch} below), and therefore are also related to misreported catch [@perretti2020Simulation]. In this analysis we consider the SCAA model, "Base", and then examine four possible models that treat the $\varepsilon_{a,y}$ deviations as random effects.

Base is a SCAA model, which estimates recruitment deviations, $\varepsilon_{1,y}$, in each year $y > 1$ as unconstrained fixed effect parameters. The *NAA* deviations for ages $a > 1$ are not estimated (i.e., $\varepsilon_{a,y} = 0$ for $a > 1$ in Eqn \ref{eq:NAA}).
...
Below, we demonstrate three models with random effect deviations on logistic selectivity, akin to those for *M*. Sel-1 treats all numbers at age as independent random effects (i.e. NAA-3) but with no random effects on *s* ($\sigma^2_s = \phi_{par} = \phi_{year} = 0$ and not estimated). Sel-2 allows IID *s* deviations, estimating $\sigma^2_s$ but fixing $\phi_{par} = \phi_{year} = 0$. Sel-3 estimates the full 2D AR(1) structure for *s* deviations.
...
For this reason, we generally prefer to model $X_y$ as a stationary AR(1) process as in @miller2018Temporal:
...
where $\mu_X$, $\sigma^2_X$, and $|\phi_X| < 1$ are the marginal mean, variance, and autocorrelation parameters. In addition to having stationary variance, another important difference between the random walk and AR(1) in short-term projections is that the AR(1) will gradually revert to the mean over time, unless environmental covariate observations are included in the projection period.
...
where $n$, $a^{*}$, and $A^{*}$ are the number of non-zero age classes, and the youngest and oldest non-zero age classes in dataset *d* and year *y* (subscripts *d* and *y* dropped for simplicity). $\mathrm{Neff}_{d,y}$ is an effective sample size that can scale the estimated observation error, $\tau_d$, for dataset *d* based on changes in sampling effort in time. Zero observations can either be pooled with adjacent non-zero age classes or treated as missing.
...
The estimated AR and IID random effects were similar, with the AR random effects slightly smoothed compared to the IID random effects when the estimated correlations were positive (e.g. $\rho_{year} = 0.73$, 95% CI: 0.58-0.87, for Georges Bank haddock in Fig. \ref{fig:devs-GBhaddock-naa}).
