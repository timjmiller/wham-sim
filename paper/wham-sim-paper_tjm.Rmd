...
where $\mu_X$, $\sigma^2_X$, and $|\phi_X| < 1$ are the marginal mean, conditional variance, and autocorrelation parameters. In addition to stationarity, another important difference between the random walk and AR(1) in short-term projections is that the AR(1) will gradually revert to the mean over time, unless environmental covariate observations are included in the projection period.
...
WHAM includes several options for the catch and index age compositions including multinomial (the default, as in ASAP), Dirichlet, Dirichlet-multinomial [@thorson2019Perspective], logistic normal [@aitchison1980LogisticNormal; @schnute2007Compositional] and zero-inflated logistic normal distributions ([Appendix]). Although the multinomial has been commonly used in stock assessment, it is generally inferior to more flexible options that are self-weighting and allow for correlations, such as the Dirichlet and logistic normal [@francis2014Replacing]. Thus, we assumed a logistic normal distribution for age composition observations in all of the applications and simulation studies here.
...
where $n$, $a^{*}$, and $A^{*}$ are the number of non-zero age classes, and the youngest and oldest non-zero age classes in dataset *d* and year *y* (subscripts *d* and *y* dropped for simplicity). $\mathrm{Neff}_{d,y}$ is an effective sample size that can scale the estimated observation error variance, $\tau_d$, for dataset *d* based on changes in sampling effort in time. Zero observations can either be pooled with adjacent non-zero age classes or treated as missing.
...
where $\phi$ > 0 is an estimated parameter.
...
### Logistic normal (as in main text)
...
where $\tau$ > 0 is an estimated parameter. The effective sample size for dataset *d* and year *y*, $\mathrm{Neff}_{d,y}$, can scale the estimated observation error, $\tau_d$, for dataset *d* based on changes in sampling effort in time.

### Zero-inflated logistic normal, inspired by zero-one inflated beta in @ospina2012General



### Zero-inflated logistic normal where $p_0$ is a function of binomial sample size
