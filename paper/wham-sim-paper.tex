...
(Cadigan 2016; Stock et al. 2021), \(s_{a,y}\) (Xu et al. 2019), and
\(N_{a,y}\) (Stock et al. 2021), as well as in the catch (\(C_{a,y}\))
...
stock-recruit function, and \emph{A} represents the plus-group. In this
analysis we consider the SCAA model, ``Base'', and then examine four
possible models that treat the NAA deviations, \(\varepsilon_{a,y}\), as
random effects.

Base is a SCAA model, which estimates recruitment deviations,
\(\varepsilon_{1,y}\), in each year \(y > 1\) as unconstrained fixed
effect parameters. The NAA deviations for ages \(a > 1\) are not
estimated (i.e., \(\varepsilon_{a,y} = 0\) for \(a > 1\) in Eqn
\ref{eq:NAA}).
...
stationary AR1 with autocorrelation parameter \(-1<\rho_{year}<1\):
...
AR1 structure as in Stock et al. (2021):
...
and \(-1<\rho_{age}<1\) and \(-1<\rho_{year}<1\) are the AR1
...
deviations (Cadigan 2016; Stock et al. 2021):
...
AR1 variance and correlation coefficients in age and year, respectively.
...
model, with no random effects on \emph{M}
(\(\sigma^2_M = \varphi_{age} = \varphi_{year} = 0\) and not estimated).
M-2 allows IID \emph{M} deviations, estimating \(\sigma^2_M\) but fixing
\(\varphi_{age} = \varphi_{year} = 0\). M-3 estimates the full 2D AR1
structure for \emph{M} deviations.
...
\(u_k\), \(\sigma^2_s\) is the AR1 variance, and \(\phi_{par}\), and
\(\phi_{year}\) are the AR1 correlation coefficients by parameter and
year.
...
random effects on \emph{s}
(\(\sigma^2_s = \phi_{par} = \phi_{year} = 0\) and not estimated). Sel-2
allows IID \emph{s} deviations, estimating \(\sigma^2_s\) but fixing
\(\phi_{par} = \phi_{year} = 0\). Sel-3 estimates the full 2D AR1
...
\(X_y\) as an AR1 process instead of a random walk (estimate
...
fixed \(\mu_{M_a}\) at the values used, or estimated as the case for
butterfish, in the original assessments. Except for the GB haddock
\emph{Sel} models, we used the same selectivity parameterization and
time blocks as in the original assessments. We used the NAA-3 model (all
\emph{NAA} deviations are random effects) in the \emph{Sel} and
\emph{Ecov} demonstrations, but the NAA-1 model (only recruitment
deviations are random effects) for the \emph{M} demonstrations, because
deviations in the \emph{NAA} transitions can be caused by unmodeled
deviations in \emph{M}. For the SNEMA yellowtail flounder \emph{Ecov}
models, we used the Cold Pool Index (CPI) as calculated in Miller et al.
(2016). We updated the CPI through 2018, except that the 2017
observation was missing because the NEFSC fall bottom trawl survey was
not completed in the SNEMA region. Within each process, we compared the
performance of the different random effects models using AIC.
...
effects was also generally supported by AIC, either the AR1 when only
recruitment deviations were random effects or the 2D AR1 when all ages
...
Including 2D AR1 autocorrelation on \emph{NAA} deviations further
...
for butterfish and SNEMA yellowtail flounder. The M-3 model with 2D AR1
autocorrelation estimated similar \emph{M} deviations as the M-2 model,
with smoothing when estimated correlations were positive (e.g.,
\(\varphi_{year} = 0.63\), 95\% CI: 0.31-0.94, and
...
Ecov-1 and Ecov-2 through Ecov-5, Fig. \ref{fig:daic}). The AR1-linear
model (Ecov-4) had the lowest AIC---fitting the CPI using an AR1 model
...
Ecov-4). As expected, the AR1 process model estimated higher uncertainty
in years with higher observation error (e.g.~1982--1992) and missing
observations (2017, Fig. \ref{fig:devs-SNEMAYT-ecov}A). The CPI
...
S11--S16). Bias was also generally small when more complex models were
...
generally negligible or negative, as expected (Figs. S17-S20).
Restricted maximum likelihood (REML) should be used if more accurate
estimation of these parameters is a priority.
...
the AR1 CPI models (Fig. S20). \(\beta_0\) was biased high in all
models, although this was not significant for the model with lowest AIC
(Ecov-4, AR1-linear). All models estimated the parameters \(\phi_X\),
...
2D AR1 random effect deviations performed well across stocks and
processes, which suggests that they warrant consideration in future
stock-specific studies.
...
treat multiple processes as random effects simultaneously. The
deviations in \emph{NAA} transitions are akin to survival, which is
related to \emph{M}, but they can also be caused by deviations in
\emph{F}, \emph{W}, or movement into or out of the stock (Gudmundsson
and Gunnlaugsson 2012; Stock et al. 2021). More precisely, they arise
from inconsistencies between the data and the equation for predicted
catch, Eqn. \ref{eq:predcatch}. For SNEMA yellowtail flounder, Stock et
al. (2021) found that models with process errors in both \emph{M} and
\emph{NAA} converged and performed best by AIC and Mohn's \(\rho\).
Models with 2D autocorrelation in either process were identifiable and
estimated similar \emph{F} and SSB trends. Preliminary work on other
stocks in the region have also shown this is possible. In general, which
combinations of process errors are expected to work jointly is difficult
to say without testing different combinations. The answer is very likely
to be stock-specific as well, depending on the information content in
the data, number of ages and years, life history, and exploitation
history.
...
Model diagnostics that would be checked in a full assessment, such as
residual patterns and mohn's rho, were not examined here, and doing so
might lead to the rejection of some model configurations.
...
al. 2021). However, the AR1 process demonstrated here trends to the mean
...
\hspace{1em}NAA-2: AR1 $R_y$ & Recruitment deviations are autocorrelated (AR1) random effects & $\sigma_R$, $\rho_{year}$ & 2\\
\hspace{1em}NAA-3: Indep. $N_{a,y}$ & All NAA deviations are independent random effects & $\sigma_R$, $\sigma_a$ & 2\\
\hspace{1em}NAA-4: 2D AR1 $N_{a,y}$ & All NAA deviations are random effects with correlation by year and age (2D AR1) & $\sigma_R$, $\sigma_a$, $\rho_{year}$, $\rho_{age}$ & 4\\
\addlinespace[0.3em]
...
\hspace{1em}M-3: 2D AR1 & $M$ deviations are random effects with correlation by year and age & $\sigma_R$, $\sigma_M$, $\varphi_{year}$, $\varphi_{age}$ & 4\\
\addlinespace[0.3em]
...
\hspace{1em}Sel-3: 2D AR1 & Selectivity deviations are random effects with correlation by year and parameter & $\sigma_R$, $\sigma_a$, $\sigma_{Sel}$, $\phi_{year}$, $\phi_{par}$ & 5\\
\addlinespace[0.3em]
...
\hspace{1em}Ecov-4: AR1-linear & Ecov: autocorrelated (AR1), effect on $\beta$: linear & $\sigma_R$, $\sigma_a$, $\sigma_x$, $\phi_x$, $\beta_1$ & 5\\
\hspace{1em}Ecov-5: AR1-poly & Ecov: autocorrelated (AR1), effect on $\beta$: 2nd order polynomial (poly) & $\sigma_R$, $\sigma_a$, $\sigma_x$, $\phi_x$, $\beta_1$, $\beta_2$ & 6\\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}
...
{\centering \includegraphics[width=6.5in]{/home/bstock/Documents/ms/wham-sim/plots/v1/into_paper/8_CPI_Recruit_SNEMAYT_Ecov2} 
...
\caption{Relative error of key quantities estimated for Georges Bank haddock using four models of numbers-at-age (NAA) random effects. NAA-1 = only recruitment deviations are random effects (most similar to Base, a traditional statistical catch-at-age model), and deviations are independent. NAA-2 = as NAA-1, but with autocorrelated recruitment deviations. NAA-3 = all NAA deviations are independent random effects. NAA-4 = as NAA-3, but deviations are correlated by age and year.}\label{fig:rel-error-GBhaddock-naa}
\end{figure}
...
\caption{Relative error of key quantities estimated for Georges Bank haddock using three models of selectivity random effects. Sel-1 = no random effects (constant logistic selectivity). Sel-2 = independent selectivity deviations. Sel-3 = selectivity deviations are correlated by parameter and year.}\label{fig:rel-error-GBhaddock-sel}
\end{figure}
