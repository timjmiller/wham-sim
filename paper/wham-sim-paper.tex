...
X is specified and 40 by default), \emph{should we quickly include an
option for projecting \(F_{MSY}\)}, user-specified \(F_y\), or \(F\)
derived from user-specified catch. For all options except user-specified
\(F_y\), the uncertainty in projected F is propogated into the
...
outputs (SSB, \emph{F}, stock and harvest status) when the operating and
estimation models matched. The less complex models, without random
effects or autoregressive structure, exhibited some bias in cross-tests,
while the more complex models did not. In these cases, bias in SSB and
\emph{F} were opposite such that predicted catch was unbiased. The WHAM
models with IID or 2D AR1 random effect deviations performed well across
stocks and processes, which suggests that they warrant consideration in
future stock-specific studies.
...
WHAM can estimate interannual transitions in NAA as random walk
processes like SAM, or as a function of autoregressive deviations.
Alternatively (or simultaneously, Stock et al., n.d.) WHAM can estimate
deviations in natural mortality as autoregressive processes like NCAM
(Cadigan, 2016). Uniquely, WHAM can model multiple environmental
covariate time series as state-space processes and include their effects
(nonlinearly) in various ways on recruitment or natural mortality.
...
frameworks in the U.S, e.g.~ASAP, Stock Synthesis (SS), an Assessment
Model for Alaska (AMAK), and the Beaufort Assessment Model (BAM), make
this same separability assumption because oftentimes we want to specify
\(F\) in projections to calculate a reference point, as opposed to
continuing a \(F\) time-series process. Estimating selectivity in time
blocks is common practice in these frameworks. In contrast, SAM
estimates \(F_{a,y}\) directly as a multivariate stochastic process
(Nielsen and Berg, 2014). The approach in WHAM of estimating \(F_y\),
\(\sigma_s\), \(\phi_y\), and \(\phi_p\) as fixed effect parameters and
allowing 2D AR1 random effects on the selectivity parameters should
allow for similar selectivity patterns as in SAM. The method in Stock
Synthesisis is similar: allow 2D AR1 random effects on \(s_{a,y}\)
instead of the parameters, and then estimate \(\sigma_s\) by an
iterative tuning algorithm (Xu et al., 2019). It is unclear whether
estimating time-varying selectivity as random effects produces different
results or an improvement over using time blocks, or if there are
advantages to using any of the three approaches for estimating
time-varying selectivity used by WHAM, SAM, or Stock Synthesis.

Likewise, WHAM and SAM also make different separability assumptions for
the catch and index observations. WHAM treats aggregate and composition
observations separately for fisheries and indices, as do ASAP, SS, AMAK,
and BAM. This observation model is natural for landings data that are
measured as total weight plus age composition sampling, with survey age
composition sampling often done separately. This differs from SAM where
\(C_{a,y}\) and \(I_{a,y}\) observations are treated directly as
...
stock, this evaluation likely needs to be conducted on a stock-by-stock
basis.
...
al., n.d.). However, the AR1 process demonstrated here reverts to the
...
time-series extremes, and conditions in the near future may have never
been observed (e.g., sea surface temperatures over the U.S. Northeast
Shelf in the last decade; Chen et al., 2020). More complex nonstationary
time-series models such as autoregressive integrated moving average
(ARIMA) that can forecast beyond observed values could be implemented.
Nonlinear (e.g.~splines) or double linear integration (Di Lorenzo and
Ohman, 2013) techniques could also be worth pursuing, as well as
time-delay embedding methods (Munch et al., 2018, 2017), which not only
allow for nonstationary dynamics but also do not require a specified
functional form. Still, stochastic projections of productivity responses
to environmental conditions that are rapidly changing and at time-series
extremes should be done with care.
...
Although including external drivers of productivity may increase model
uncertainty without improving accuracy, stock assessments that do not
allow for time-varying productivity underestimate model uncertainty and
implicitly assume that conditions will return to average in forecasts.
In lieu of operationalizing an environment-linked WHAM model to set
catch advice, WHAM can still be useful in two main ways. First, WHAM can
serve as an operating model to simulate realistic environmentally-driven
changes in recruitment or \emph{M} in order to estimate the sensitivity
of status quo models. Second, WHAM could account for uncertainty in
environment-productivity relationships by constructing ensembles of
...
stocks in a changing environment. We forsee environment-linked stock
assessments increasingly becoming feasible as fisheries and
...
assessment models such as WHAM, allowing us to treat population and
environmental processes as time-varying random effects in a state-space
framework.
