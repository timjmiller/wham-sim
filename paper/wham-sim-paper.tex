...
\(F_{a,y}\), \(W_{a,y}\), or movement into or out of the stock (Schnute
and Richards 1995; Gudmundsson and Gunnlaugsson 2012; Stock et al.
2021). More precisely, they arise from inconsistencies between the data
and the model predicted catch (i.e.~Eqn. \ref{eq:predcatch} below), and
therefore are also related to misreported catch (Perretti et al. 2020).
In this analysis we consider the SCAA model, ``Base'', and then examine
four possible models that treat the \(\varepsilon_{a,y}\) deviations as
random effects.
...
\(\varphi_{age} = \varphi_{year} = 0\). M-3 estimates the full 2D AR(1)
...
\(\phi_{par} = \phi_{year} = 0\). Sel-3 estimates the full 2D AR(1)
...
AR(1) process as in Miller et al. (2018):
...
walk and AR(1) in short-term projections is that the AR(1) will
...
size that can scale the estimated observation error, \(\tau_d\), for
dataset \emph{d} based on changes in sampling effort in time. Zero
observations can either be pooled with adjacent non-zero age classes or
treated as missing.
