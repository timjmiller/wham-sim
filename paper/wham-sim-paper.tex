...
(Northern cod: Shelton et al., 2006, @rose2015Northern; Gulf of Maine
...
penalty terms, \(\sigma^2_\epsilon\), must be fixed and are therefore
subjective (Methot and Wetzel, 2013; Xu et al., 2019). State-space
...
constraining random effects (Aeberhard et al., 2018). In this way,
state-space models can allow processes to vary in time while
simultaneously estimating fewer parameters.
...
medium-term (3-10 years) for stocks that have not responded to dramatic
decreases in \(F\). Miller et al. (2016) developed a state-space model
for yellowtail flounder with an environmental effect on recruitment,
which reduced retrospective patterns and residual variance. Similar
applications that include environmental effects on growth, \(M\), and
maturity in a state-space framework have also proven promising (Miller
and Hyun, 2018; Miller et al., 2018; O'Leary et al., 2019; Xu et al.,
2018).

In summary, to address the needs of fisheries management in a changing
climate we seek an assessment framework that 1) estimates time-varying
parameters as random effects (i.e.~a state-space model), and 2) includes
environmental effects. The framework should also allow for easy testing
against status quo SCAA models to ease gradual adoption through the
``research track'' or ``benchmark'' assessment process (Lynch et al.,
2018). The objectives of this manuscript are to introduce the Woods Hole
Assessment Model (WHAM) framework and demonstrate its ability to:
...
\item
  ...
  (\emph{sensu} Deroba et al., 2015).
\end{enumerate}

Finally, we describe how the above are implemented using an open-source
software package available to the stock assessment community (Miller and
Stock, 2020).
...
\hypertarget{modules-with-random-effects}{%
\subsubsection{Modules with random
effects}\label{modules-with-random-effects}}
...
2D AR1 structure performed well across modules and stocks (Fig.
\ref{fig:daic}).
...
\hypertarget{selectivity-2}{%
\subsubsection{Selectivity}\label{selectivity-2}}
