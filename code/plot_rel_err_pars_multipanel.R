  ...
  for(ni in 1:length(id.list)){
    ...
    if(n.stocks > 0){
      ...
      for(ty in sim.types){
        ...
        if(names(id.list)[ni] == "Ecov2") png(file.path(plots_dir_i, paste0("estpar_",names(id.list)[ni],"_",types[ty],".png")), width=10, height=3, units='in',res=300)
        if(names(id.list)[ni] == "sel") png(file.path(plots_dir_i, paste0("estpar_",names(id.list)[ni],"_",types[ty],".png")), width=7, height=3, units='in',res=300)
