...
plot_rel_err_pars_multipanel <- function(ids, re, bc.type=2, sim.types=1:2, plot.eps=FALSE,
  ...
  ind.ecov <- which(re == "Ecov")
  if(length(ind.ecov) == 0) id.ecov <- NULL else id.ecov <- paste0(ids[ind.ecov],"_Ecov")
  ind.sel <- which(re == "sel")
  if(length(ind.sel) == 0) id.sel <- NULL else id.sel <- paste0(ids[ind.sel],"_sel")
  id.list <- list(NAA = id.naa, M = id.m, Ecov=id.ecov, sel=id.sel)
  n.sim = 100
  ...
  for(ni in 1:length(id.list)){
    ...
    if(n.stocks > 0){
      ...
      if(names(id.list)[ni] == "Ecov"){
        par.labs <- c("sigR","sigA","sigX","phi","beta")
        par.labs.expr <- c(expression(sigma[R]), expression(sigma[a]), expression(sigma[x]), expression(varphi[x]), expression(beta))
      }
      if(names(id.list)[ni] == "sel"){
        par.labs <- c("sigR","sigA","sigS","phiY","phiA")
        par.labs.expr <- c(expression(sigma[R]), expression(sigma[a]), expression(sigma[Sel]), expression(varphi[y]), expression(varphi[a]))
      }
      ...
      for(ty in sim.types){
        ...
        if(n.pars == 5 & names(id.list)[ni] == "Ecov") png(file.path(plots_dir_i, paste0("estpar_",names(id.list)[ni],"_",types[ty],".png")), width=4, height=3, units='in',res=300)
        if(n.pars == 5 & names(id.list)[ni] == "sel") png(file.path(plots_dir_i, paste0("estpar_",names(id.list)[ni],"_",types[ty],".png")), width=7, height=3, units='in',res=300)
