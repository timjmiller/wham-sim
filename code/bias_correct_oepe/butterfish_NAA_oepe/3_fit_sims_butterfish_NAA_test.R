...
simdata_dir <- here("data","simdata","butterfish_NAA")
...
# for(m in 1:n.mods){
	...
	for(i in 1:n.sim){
		...
		# input1$par$logit_selpars[1,1:2] <- input1$par$logit_selpars[4,3] <- 0
		# fit1 <- fit_wham(input1, do.sdrep=F, do.osa=F, do.retro=F, do.proj=F, do.check=T)
