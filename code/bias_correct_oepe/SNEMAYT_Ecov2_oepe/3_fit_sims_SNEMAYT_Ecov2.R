...
# for(m in 1:n.mods){
	...
	for(i in 1:n.sim){
		...
		ecov.mod <- input1$data$Ecov_model
		ecov.how <- input1$data$Ecov_how
		ecov.poly <- input1$data$Ecov_poly
		n.data <- length(input1$data)
		input1$data <- simdata[[i]][[1]]
		input1$data$Ecov_model = ecov.mod
		input1$data$Ecov_how = ecov.how
		input1$data$Ecov_poly = ecov.poly	
		...
		n.data <- length(input2$data)
		input2$data <- simdata[[i]][[2]]
		input2$data$Ecov_model = ecov.mod
		input2$data$Ecov_how = ecov.how
		input2$data$Ecov_poly = ecov.poly
