...
# source("/home/bstock/Documents/ms/wham-sim/code/bias_correct_oepe/SNEMAYT_Ecov_oepe/4_results_SNEMAYT_Ecov.R")
...
plots_dir <- here("plots","bias_correct_oepe","SNEMAYT_Ecov")
...
types <- c("OE","OEPE")
results$om <- factor(results$om, levels=1, labels=c("m1: CPI-Recruitment"))
results$em <- factor(results$em, levels=1, labels=c("m1: CPI-Recruitment"))
results$type <- factor(results$type, levels=1:2, labels=c("Simulated data: Obs error", "Simulated data: Obs + Process error (new NAA)"))
results$em.x <- fct_recode(results$em, m1="m1: CPI-Recruitment")
...
simdata <- lapply(1, function(x) readRDS(here("data","simdata","bias_correct_oepe","SNEMAYT_Ecov_oepe",paste0("simdata_om",x,".rds"))))
