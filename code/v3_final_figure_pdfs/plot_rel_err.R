...
# stock.id=ids[j]; re=re[j]; bc.type=bc.type; sim.types=2; ty=1; multipanel=TRUE
plot_rel_err <- function(results, stock.id="SNEMAYT", re="NAA", bc.type=2, sim.types=2, multipanel=TRUE, plot.eps=FALSE,
  ...
  if(multipanel){
    ...
    # collapse across years, group by om/em
      ...
      # cols <- cols[-1]
        ...
        # facet_grid(rows=vars(variable), cols=vars(om2)) +
    	  theme_bw() +
    	  # theme(legend.position="bottom",
    	  theme(legend.position="none",
    	        strip.text.y = element_text(size = 10), strip.text.x = element_text(size = 10, margin = margin(3,1,1,1, "pt")),
    	        axis.title.x = element_text(size = 12), axis.text.x = element_text(size = 8, angle = 45, vjust = 1, hjust=1),
      ...
      for (i in stripr) {
      j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
      g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- "white"
      }

    	title <- ggdraw() + draw_label("Operating model", hjust = 0.3, vjust=0.9) + theme(plot.margin = margin(0, 0, 0, 0))
      p1 <- plot_grid(title, g, ncol = 1, rel_heights = c(0.045, 1))

      # if(n.mods == 5) png(file.path(plots_dir, paste0(id,"_medianCI_",types[ty],".png")), height=8, width=7, units='in', res=300)
      # if(n.mods == 4) png(file.path(plots_dir, paste0(id,"_medianCI_",types[ty],".png")), height=8, width=6.5, units='in', res=300)
      # if(n.mods == 3) png(file.path(plots_dir, paste0(id,"_medianCI_",types[ty],".png")), height=8, width=5, units='in', res=300)
      # if(n.mods == 2) png(file.path(plots_dir, paste0(id,"_medianCI_",types[ty],".png")), height=8, width=4, units='in', res=300)
      # if(n.mods == 1) png(file.path(plots_dir, paste0(id,"_medianCI_",types[ty],".png")), height=8, width=3, units='in', res=300)
      # print(p1)
      # dev.off()
      return(p1)
    }
