...
simdata <- lapply(1:4, function(x) readRDS(file.path(res_dir,paste0("simdata_om",x,".rds"))))
results$R.sim = NA
for(om in 1:4){
  for(em in 1:4){
    for(i in 1:100){
...
# collapse across years, group by om/em
  ...
  png(file.path(res_dir,"SNEMAYT_NAA_test.png"),height=8.5, width=6,res=300,units='in')
...
# collapse across years, group by om/em
  ...
  png(file.path(res_dir,"SNEMAYT_NAA_test_oe.png"),height=8.5, width=6,res=300,units='in')
   p <- ggplot(df.plot, aes(x=em.x, y=val)) +
                geom_boxplot(aes(fill=em2), outlier.shape = NA) +
                scale_fill_jco(name="", labels=lapply(levels(df.plot$em2), function(x) parse(text=x))) +
                coord_cartesian(ylim=c(-1,1)) +
...
# collapse across years, group by om/em
  df.plot <- filter(results, type==levels(results$type)[ty]) %>%
              select(om, em, em.x, SSB.rel.bc, F.rel.bc, relB.rel.bc, relF.rel.bc, R.rel.bc) %>%
              pivot_longer(-c(om,em,em.x), names_to = "variable", values_to = "val") %>%
              group_by(om, em)
  df.plot$val = df.plot$val - 1 # relative error
  
  df.plot$variable <- factor(df.plot$variable, levels=c("SSB.rel.bc", "F.rel.bc", "relB.rel.bc", "relF.rel.bc", "R.rel.bc"), 
                         labels=c("SSB", "F", expression(B/B[40]["%"]), expression(F/F[40]["%"]), "Recruitment"))
  df.plot$om2 <- factor(df.plot$om, labels=c(expression(paste("m1:")~paste("SCAA")~paste("(iid)")), 
                                       expression(paste("m2:")~paste("SCAA (")*AR1[y]*paste(")")), 
                                       expression(paste("m3:")~paste("NAA")~paste("(iid)")), 
  ...
  png(file.path(res_dir,"SNEMAYT_NAA_test_oepe_bc.png"),height=8.5, width=6,res=300,units='in')
   p <- ggplot(df.plot, aes(x=em.x, y=val)) +
                geom_boxplot(aes(fill=em2), outlier.shape = NA) +
                scale_fill_jco(name="", labels=lapply(levels(df.plot$em2), function(x) parse(text=x))) +
                coord_cartesian(ylim=c(-1,1)) +
...
# collapse across years, group by om/em
  ...
  png(file.path(res_dir,"SNEMAYT_NAA_test_oe_bc.png"),height=8.5, width=6,res=300,units='in')
   p <- ggplot(df.plot, aes(x=em.x, y=val)) +
                geom_boxplot(aes(fill=em2), outlier.shape = NA) +
                scale_fill_jco(name="", labels=lapply(levels(df.plot$em2), function(x) parse(text=x))) +
                coord_cartesian(ylim=c(-1,1)) +
