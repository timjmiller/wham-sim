...
simdata_dir <- here("data","simdata","ICEherring_M")
...
simdata <- readRDS(file.path(simdata_dir,"simdata_om1.rds"))
...
# for(m in 1:n.mods){
	...
	for(i in 1:n.sim){
		...
		input1 <- readRDS(file.path(simdata_dir,paste0("m",em,"_input.rds")))
		...
		est.selpars <- which(is.finite(input1$par$logit_selpars))
		tmp <- rep(NA, length(input1$par$logit_selpars))
		tmp[est.selpars] <- 1:length(est.selpars)
		input1$map$logit_selpars <- factor(tmp)
		...
		input2 <- readRDS(file.path(simdata_dir,paste0("m",em,"_input.rds")))
		...
		est.selpars <- which(is.finite(input2$par$logit_selpars))
		tmp <- rep(NA, length(input2$par$logit_selpars))
		tmp[est.selpars] <- 1:length(est.selpars)
		input2$map$logit_selpars <- factor(tmp)
