...
n.sim <- 100
n.types <- 2
simdata <- readRDS("simdata_SNEMAYT_M_om1.rds")
...
# for(m in 1:n.mods){
	simdata <- readRDS(file.path(simdata_dir,paste0("simdata_SNEMAYT_M_om",om,".rds")))
	for(i in 1:n.sim){
		# print(paste0("Model: ",m," Sim: ", i))
		print(paste0("OM: ",om," EM: ",em," Sim: ", i))
		set.seed(sim.seeds[i])
		results <- rep(list(matrix(NA, ncol = length(res.colnames), nrow = n.years)),n.types) # nested lists with preallocated matrices
		sdreps <- nested.list(n.types) # sdreps with analytical bias correction
		reps <- nested.list(n.types)
		...
		input1$data$M_re_model <- c(1,2,5)[em]
		...
		if(!'err' %in% names(fit1) & class(fit1) != "character"){
			...
			if(class(fit1$sdrep) == "sdreport"){
				...
				sdreps[[1]] <- s1
				results[[1]] <- tryCatch(calc_results(om=om, em=em, type=1, sim=i, fit1, s1),
					error = function(e) conditionMessage(e))
			} else {
				results[[1]] <- "Error: sdreport failed, no results to calculate"
				sdreps[[1]] <- fit1$sdrep # error message
			}
		} else {
			results[[1]] <- "Error: model did not converge, no results to calculate"
			if(class(fit1) != "character") reps[[1]] <- fit1$err # error message
			if(class(fit1) == "character") reps[[1]] <- fit1
			sdreps[[1]] <- "Error: model did not converge, sdreport not attempted"
		}
		...
		input2$data$M_re_model <- c(1,2,5)[em]
		...
		if(!'err' %in% names(fit2) & class(fit2) != "character"){
			...
			if(class(fit2$sdrep) == "sdreport"){
				...
				sdreps[[2]] <- s2
				results[[2]] <- tryCatch(calc_results(om=om, em=em, type=2, sim=i, fit2, s2),
					error = function(e) conditionMessage(e))
			} else {
				results[[2]] <- "Error: sdreport failed, no results to calculate"
				sdreps[[2]] <- fit2$sdrep # error message
			}
		} else {
			results[[2]] <- "Error: model did not converge, no results to calculate"
			if(class(fit1) != "character") reps[[2]] <- fit2$err # error message
			if(class(fit1) == "character") reps[[2]] <- fit2
			sdreps[[2]] <- "Error: model did not converge, sdreport not attempted"
		}

		# saveRDS(results, file=here("results","NAA",paste0("res_om",m,"_sim_",i,".rds")))
		rm(list=c("input1","input2","fit1","fit2","df","dfnaa","dfnaa.bc"))

		saveRDS(results, file=paste0("results_SNEMAYT_M_om",om,"_em",em,"_sim",i,".rds"))	
		saveRDS(sdreps, file=paste0("sdreps_SNEMAYT_M_om",om,"_em",em,"_sim",i,".rds"))	
		saveRDS(reps, file=paste0("reps_SNEMAYT_M_om",om,"_em",em,"_sim",i,".rds"))	
		rm(list=c("results","sdreps","reps"))
	}
	rm(list=c("simdata"))
# }
