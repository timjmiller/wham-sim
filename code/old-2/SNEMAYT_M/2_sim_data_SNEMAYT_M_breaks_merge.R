...
# source(here::here("code","SNEMAYT_M","2_sim_data_SNEMAYT_M_breaks_merge.R"))
...
for(om in 1:n.mods){
	simdata <- list()
	for(b in 1:n.b){
		simdata_b <- readRDS(file.path(out_dir,paste0("simdata_SNEMAYT_M_om",om,"_",b,".rds")))
		simdata <- c(simdata, simdata_b)
	}
	saveRDS(simdata, file=file.path(out_dir,paste0("simdata_SNEMAYT_M_om",om,".rds")))
}
